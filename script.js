        ...
        switch (source) {
          ...
          default:
            ...
            body = `震源地: ${
              ...
              data.MaxIntensity || data.intensity || data.epiIntensity
            }`;
        }
      ...
      // 中国地震局（CEA）接続関数
      function connectCea() {
        if (ceaWs) ceaWs.close();
        ceaWs = new WebSocket("wss://ws.fanstudio.tech/cea");
        ...
        ceaWs.onopen = () => {
          ...
          if (ceaWs.readyState === WebSocket.OPEN) {
            ceaWs.send("query_cea");
          }
        };

        ceaWs.onmessage = (event) => {
          try {
    ...
    `;
      ...
      async function fetchJmaXmlData() {
        try {
          ...
          entries.forEach((entry) => {
            const title =
              entry.querySelector("title")?.textContent || "情報なし";
            const link =
      ...
      function updateCombinedDisplay() {
        ...
        allData.forEach((item) => {
          ...
          // JMA緊急地震速報表示
            ...
            // 震度表示
            if (item.MaxIntensity) {
              html += `<p>最大震度: ${getIntersityLabel_j(
                item.MaxIntensity
              )}</p>`;
          ...
          // JMA地震情報リスト表示
            ...
            html += `<p class="time">発生時刻: ${
              item.time_full || item.time
            }</p>`;
            html += `<p class="location">震源地: ${item.location}</p>`;
            html += `<p>マグニチュード: ${item.magnitude}</p>`;
        ...
        const latestTime = new Date(
          Math.max(
            ...Object.values(lastUpdateTimes).filter((time) => time !== null)
          )
        );
      ...
      async function initialJmaXmlFetch() {
        ...
        // 5分後に再取得を試行
        setTimeout(fetchJmaXmlData, 300000);
      }
    ...
    `;
      ...
      function startAutoFetch() {
        ...
        autoFetchInterval = setInterval(() => {
          ...
          // 中国地震局（CEA）データ
          if (connections.cea && ceaWs?.readyState === WebSocket.OPEN) {
            ceaWs.send("query_cea");
          }

          // 成都高新防災減災研究所（ICL）データ（公開ソフトウェアでの使用禁止）
          if (connections.icl && iclWs?.readyState === WebSocket.OPEN) {
            iclWs.send("query_icleew");
          }
          // USGSデータ
      ...
      function parseJmaXmlDetail(xml) {
        ...
        if (earthquake) {
          ...
          const hypocenter =
            ...
            "情報なし";
          const coordinate =
            earthquake.querySelector("Hypocenter > Area > Coordinate")
              ?.textContent ||
            earthquake.querySelector("hypocenter > area > coordinate")
              ?.textContent ||
            "情報なし";
        ...
        if (observation) {
          ...
          prefList.forEach((pref) => {
            const prefName =
              pref.querySelector("Name")?.textContent || "情報なし";
            const prefMaxInt =
            ...
            areaList.forEach((area) => {
              const areaName =
                area.querySelector("Name")?.textContent || "情報なし";
              const areaMaxInt =
              ...
              html += `<div style="margin-left: 20px;">`;
              html += `<p>・${areaName}: ${getIntersityLabel_j(
                areaMaxInt
              )}</p>`;
              ...
              cityList.forEach((city) => {
                ...
                html += `<p>└─ ${cityName}: ${getIntersityLabel_j(
                  cityMaxInt
                )}</p>`;

                // 観測点の震度
                const stationList =
                  city.querySelectorAll("IntensityStation") || [];
